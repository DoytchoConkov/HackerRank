...
class SumInLeavesVisitor extends TreeVis {
    ...
    public void visitNode(TreeNode node) {
        //implement this
        //  sumVisitors+=node.getValue();
    }

    public void visitLeaf(TreeLeaf leaf) {
        sumVisitors += leaf.getValue();
        //implement this
    }
}
...
class ProductOfRedNodesVisitor extends TreeVis {
    ...
    public void visitNode(TreeNode node) {
        ...
        if (node.getColor() == Color.RED)
            products = (products * node.getValue()) % 1000000007;
    }

    public void visitLeaf(TreeLeaf leaf) {
        //implement this
        if (leaf.getColor() == Color.RED)
            products = (products * leaf.getValue()) % 1000000007;
    }
}
...
public class JavaVisitorPattern {
    ...
    public static Tree solve() {
        ...
        for (int i = 0; i < n; i++) {
            nodeAtts.put(i + 1, new Object[]{scanner.nextInt(), null});
        }
        for (int i = 0; i < n; i++) {
            if (scanner.nextInt() == 0) {
                nodeAtts.get(i + 1)[1] = Color.RED;
            } else {
                nodeAtts.get(i + 1)[1] = Color.GREEN;
            }
        }
